...
TORQUEView was developed using the Qt development environment to allow the application to be built and run on a variety of platforms, such as Windows, Linux, and Mac OS.
...
TORQUEView can be run by users either with or without admin rights (i.e., "root" or "non-root" users). Non-root users will have admin-specific capabilities disabled.
...
The "Data source" combobox lets you choose which TORQUE data to access and display in the TORQUEView tabs. With this you can analyze either a local host, a remote host, or a snapshot file.  

When Remote Host is selected in the combobox, you choose the remote server to analyze from the Remote Server combobox to the immediate right (SSH key-based authentication is required for access to remote servers). Specify which remote hosts to include in your list by clicking the "Edit" button next to the combobox.

When Snapshot is selected, you indicate the name of the snapshot file to load from the associated edit field (this reads in data from a snapshot file and updates each of the tab windows from that data).
...
The Snapshot feature is intended primarily as a diagnostic tool so users experiencing problems can take a picture of their system for analysis. The "Snapshot" dialog lets you save all the current TORQUEView screen display data to a file. You can take either a:
- single snapshot, or
- a series of snapshots, where you specify the base-filename of the set (date & time stamps will be added to each filename), and where you indicate the time interval between snapshots.
...
The "Replay Snapshots" button lets you replay a collection of related snapshots, indicating the time interval between snapshot displays (this value is totally independent of the interval used when generating the snapshot series). After you click the "..." browse button, you only need to select one filename from the set-- the "File to be Replayed" list will be refreshed using all filenames having that same prefix. Then to start the replay, click OK.

Once the replay is started, the user can control the playback with the Pause, Resume, Next, Back, and Cancel buttons on the statusbar.
...
The "heat map" tab displays a table of all nodes on the server, showing the current state of each node as a color-coded icon. It can show nodes either with or without names (labels). It is most useful when displaying a large number of nodes and their states, letting the user see the state of the entire system at a glance.
...
The "Show nodes running job ID" checkbox is a filter that will show only the nodes running that specific job, and will highlight those job IDs in yellow. Unchecking it once again displays the entire list of nodes, but will continue to highlight the matching job IDs in yellow.
...
When right-clicking on a node in either the heat map or nodes tab, a context menu lets users:
...
The pbs_server tab provides an interface to several pbs_server commands and shows their results.

On the right is a panel that shows the contents of the "nodes" file on the current host (either local or remote), which can be edited if desired by clicking the "Edit" button.  NOTE: Since "nodes" files are so critical to cluster operation, TORQUEView will automatically create a backup of the current "nodes" file before overwriting it (the backup copy is located in the same directory, with a date/time stamp appended).
